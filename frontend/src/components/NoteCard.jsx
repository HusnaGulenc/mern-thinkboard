...
const NoteCard = ({ note, setNotes }) => {
  const handleDelete = async (e, id) => {
    ...
    try {
      ...
      setNotes((prev) => prev.filter((note) => note._id !== id)); //filter notes after deleting notes
    } catch (error) {
  ...
  return (
    <Link
      to={`/note/${note._id}`}
      className="card bg-base-100 border-t-4 border-[#00FF9D] shadow-md hover:shadow-xl transition-all duration-200"
    >
